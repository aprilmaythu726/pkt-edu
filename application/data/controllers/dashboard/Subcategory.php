...
class Subcategory extends CI_Controller
{
	...
	public function index()
	{
		$globalHeader = array(
			...
			'title' => "Subcategory",
			'msg' => "",
			'uri' => array("category","subcategory_lists"),
			'config' => $this->user_config
		);
		
		$list = $this->Subcategory_Model->categoryList();
        ...
        $data = $this->__Xss($data);
				...
				if ($checkdata){
					...
					redirect('adm/portal/subcategory');
        }
	...
	public function edit($id)
	{
		...
		$list = $this->Subcategory_Model->detail($id);
		//*** Current query value checker
		$this->__resultEmptyChecker($id, $globalHeader,"adm/portal/subcategory", $list);
		//*** Generate necessary key and value
        ...
        $data = array(
	        ...
	        'updated_at' => date("Y-m-d H:i:s"),
				...
				if ($checkdata){
					$this->session->set_flashdata('msg_error', 'Your data already exits! please fill other data!');
					redirect('adm/portal/subcategory/edit/'.$id);
        }

				$data['result'] = $this->Subcategory_Model->categoryUpdate($data, $id);
	...
	public function delete($id)
	{
		...
		$this->__resultEmptyChecker($id, $globalHeader, "adm/portal/subcategory", $globalHeader);
