...
class Category extends CI_Controller
{
	...
	public function index()
	{
		$globalHeader = array(
			...
			'title' => "Categories",
			'msg' => "",
			'uri' => array("category","category_lists"),
			'config' => $this->user_config
		);
		
		$list = $this->Category_Model->categoryList();
		...
		if($_POST) {
			$this->form_validation->set_rules('name', 'category name', 'trim|required|xss_clean|min_length[2]|is_unique[OSL_category.name]');
			$this->form_validation->set_error_delimiters("","");
			
			if ($this->form_validation->run() === false) {
				$this->load->view('dashboard/category/list', $this->data);
			} else {
        $category_name = $this->mainconfig->_strToLower($this->input->post('name'));
	...
	public function edit($id)
	{
		...
		$list = $this->Category_Model->detail($id);
		//*** Current query value checker
		$this->__resultEmptyChecker($id, $globalHeader,"adm/portal/category", $list);
		//*** Generate necessary key and value
            ...
            'status' => $this->input->post('status'),
	          'updated_at' => date("Y-m-d H:i:s"),
				...
				if ($checkdata){
					...
					redirect('adm/portal/category/edit/'.$id, $data);
        }

				$data['result'] = $this->Category_Model->categoryUpdate($data, $id);
	...
	public function delete($id)
	{
		...
		$this->__resultEmptyChecker($id, $globalHeader, "adm/portal/category", $globalHeader);
