...
class Langschoolapplicant extends CI_Controller 
{
    ...
    public function add() 
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === false) {
                ...
                if($jp_name != Null){
                $data_lang_study = [];
                $data_lang_study = array('jp_name' => $jp_name, 'jp_level' => $jp_level,'jp_status' => $jp_status,'jp_address' => $jp_address, 'jp_start_date' => $jp_start_date, 'jp_end_date' => $jp_end_date, 'jp_hour' => $jp_hour,);
                }
    ...
    public function confirm() 
    {
        ...
        $data['data_lang_study'] = array('jp_name' => $this->session->userdata('__initial_regist_data4') ['jp_name'],'jp_level' => $this->session->userdata('__initial_regist_data4') ['jp_level'],'jp_status' => $this->session->userdata('__initial_regist_data4') ['jp_status'], 'jp_address' => $this->session->userdata('__initial_regist_data4') ['jp_address'], 'jp_start_date' => $this->session->userdata('__initial_regist_data4') ['jp_start_date'], 'jp_end_date' => $this->session->userdata('__initial_regist_data4') ['jp_end_date'], 'jp_hour' => $this->session->userdata('__initial_regist_data4') ['jp_hour']);
    ...
    public function store() 
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === false) {
                ...
                for ($i = 0;$i < $count2;$i++) {
                    if($jp_name[$i] != Null){
                    $data_lang_study = array('applicant_id' => $applicant_id, 'jp_name' => $jp_name[$i],'jp_level' => $jp_level[$i],'jp_status' => $jp_status[$i], 'address' => $jp_address[$i], 'start_date' => $jp_start_date[$i], 'end_date' => $jp_end_date[$i], 'hour' => $jp_hour[$i],);
                ...
                for ($i = 0;$i < $count4;$i++) {
                    if($fam_name[$i] != Null){
                    $data_family_member = array('applicant_id' => $applicant_id, 'fam_name' => $fam_name[$i], 'fam_age' => $fam_age[$i],'fam_gender' => $fam_gender[$i],'fam_relationship' => $fam_relationship[$i], 'work_place' => $fam_work_place[$i], 'birthday' => $fam_birthday[$i], 'occupation' => $fam_occupation[$i], 'annual_income' => $fam_annual_income[$i], 'address' => $fam_address[$i], 'length_sevice' => $fam_length_sevice[$i]);
